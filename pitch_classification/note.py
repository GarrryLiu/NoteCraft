...
class Note:
    ...
    def detect_position_on_staff(self, staff, blob):
        ...
        for line_no in range(5, 8):
            distances_from_lines.append(
                (
                    2 * line_no,
                    distance(
                        (x, y), (x, staff.min_range + line_no * staff.lines_distance)
                    ),
                )
            )
        ...
        if (
            distances_from_lines[1][1] - distances_from_lines[0][1]
            <= NOTE_PITCH_DETECTION_MIDDLE_SNAPPING
        ):
    ...
    def detect_pitch(self, position_on_staff):
        ...
        return (
            self.violin_key[position_on_staff]
            if self.clef == "bass"
            else self.violin_key[position_on_staff]
        )
    ...
    def draw_notes_pitch(image, notes):
        ...
        cv2.imwrite("output/7_with_pitch.png", im_with_pitch)
