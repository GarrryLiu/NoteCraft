...
def detect_blobs(input_image, staffs):
    ...
    _, im_inv = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY_INV)  # 二值化并反色
    kernel_height = max(1, int(im_inv.shape[0] / 20))  # 减小分母，增大内核高度
    ...
    kernel_width = max(1, int(im_inv.shape[1] / 100))  # 最小值为 1
    ...
    if len(im_with_blobs.shape) == 2:  # 如果是灰度图
