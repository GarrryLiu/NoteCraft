...
def configure_blob_detector():
    """
    Configures and returns a SimpleBlobDetector with predefined parameters.
    """
...
def detect_and_draw_blobs(image):
    """
    Detects blobs in the given image and draws them onto the image.
    
    :param image: Input image for blob detection.
    :return: List of detected keypoints and image with drawn blobs.
    """
    ...
    cv2.drawKeypoints(im_with_blobs, keypoints, im_with_blobs, color=(0, 0, 255),
...
def detect_blobs(input_image, staffs):
    ...
    keypoints = configure_blob_detector(im_with_blobs)
