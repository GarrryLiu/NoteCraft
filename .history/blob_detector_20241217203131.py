...
def remove_lines(image, line_orientation="horizontal"):
    ...
    if line_orientation == "horizontal":
        kernel_height = max(1, int(image.shape[0] / 20))  # Dynamic kernel height
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, kernel_height))
    elif line_orientation == "vertical":
        kernel_width = max(1, int(image.shape[1] / 100))  # Dynamic kernel width
    ...
    kernel_height = max(1, int(im_inv.shape[0] / 20))  # Kernel size based on image height
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, kernel_height))
    ...
    kernel_width = max(1, int(im_inv.shape[1] / 100))  # Kernel size based on image width
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_width, 1))
