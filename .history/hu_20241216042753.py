...
def get_clef(image, staff, extra_down_factor=0.3):
    """
    Gets the clef from the first staff and expands downward if needed.

    :param image: Binary image to get the clef from.
    :param staff: First staff containing staff lines information.
    :param extra_down_factor: Fraction of staff height to expand downward.
    :return: Cropped clef image.
    """
    i = 0
    ...
    up = max(0, staff.lines_location[0] - int(window_height * 0.2))  # 上边界增加 20% 缓冲
    down = min(image_height, staff.lines_location[-1] + int(window_height * (0.2 + extra_down_factor)))  # 增加额外向下扩展

    # 窗口宽度计算
    key_width = int(window_height * 1.5)  # 宽度为谱表高度的1.5倍
    ...
    while i + key_width < image_width:
        ...
        # 判断窗口中黑色像素比例是否足够
        black_pixel_ratio = (np.sum(window == 0) / window.size)  # 计算黑色像素的比例
